...
def get_val_loss(model, data, device):
    model.eval()
    out = model(data)
    loss = balanced_focal_loss(out[data.test_mask], data.y[data.test_mask], alpha=1.0, reduction="sum")
    model.train()
...
def train(model, data, device, epochs):
    ...
    for epoch in range(epochs):
        ...
        loss = balanced_focal_loss(out[data.train_mask], data.y[data.train_mask], alpha=1.0, reduction="sum")
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    edge_index_PN = np.loadtxt('NN3_1_9_balance_edge_index.csv', delimiter=',')
